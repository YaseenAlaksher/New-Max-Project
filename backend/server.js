...
const JWT_SECRET = process.env.JWT_SECRET || 'newmax-secret-change-me-in-production';
...
function productToStoreFormat(p) {
  ...
  const colorMap = {};
  variants.forEach(v => { colorMap[v.color_name] = { name: v.color_name, nameAr: v.color_name_ar, hex: v.color_hex }; });
  return {
    id: p.id,
...
app.get('/api/products', auth, (req, res) => {
  ...
  if (search) {
    const q = '%' + search + '%';
    rows = db.prepare('SELECT * FROM products WHERE name_en LIKE ? OR name_ar LIKE ? ORDER BY created_at DESC').all(q, q);
  } else {
  ...
  const result = rows.map(p => ({
    ...
    variants: db.prepare('SELECT * FROM product_variants WHERE product_id = ? ORDER BY size, color_name').all(p.id),
    totalStock: db.prepare('SELECT SUM(stock) as t FROM product_variants WHERE product_id = ?').get(p.id)?.t || 0,
  }));
  res.json(result);
});
...
app.post('/api/products', auth, (req, res) => {
  const d = req.body;
  if (!d.name_en || !d.price) return res.status(400).json({ error: 'name_en and price are required' });
  ...
  `).run(
    ...
    d.name_en, d.name_ar || '',
    d.desc_en || '', d.desc_ar || '',
    d.price, d.original_price || null,
    d.category || 'tops', d.badge || '',
    d.rating ?? 4.5, d.review_count ?? 0,
  ...
  if (Array.isArray(d.variants) && d.variants.length) {
    ...
    const insertVariants = db.transaction(() =>
      d.variants.forEach(v => stmt.run(id, v.size, v.color_name, v.color_name_ar || '', v.color_hex || '#000000', v.stock || 0))
    );
    insertVariants();
  }
...
app.put('/api/products/:id', auth, (req, res) => {
  ...
  if (Array.isArray(d.variants)) {
    ...
    `);
    const insertVariants = db.transaction(() =>
      d.variants.forEach(v => stmt.run(req.params.id, v.size, v.color_name, v.color_name_ar || '', v.color_hex || '#000000', v.stock || 0))
    );
    insertVariants();
  }
...
app.put('/api/inventory/:id', auth, (req, res) => {
  ...
  if (variant) {
    const total = db.prepare('SELECT SUM(stock) as t FROM product_variants WHERE product_id = ?').get(variant.product_id)?.t || 0;
    db.prepare('UPDATE products SET in_stock = ? WHERE id = ?').run(total > 0 ? 1 : 0, variant.product_id);
  }
...
app.post('/api/inventory/variants', auth, (req, res) => {
  const { product_id, size, color_name, color_name_ar, color_hex, stock } = req.body;
  if (!product_id || !size || !color_name) return res.status(400).json({ error: 'product_id, size, color_name required' });

  db.prepare(`
    INSERT OR REPLACE INTO product_variants (product_id, size, color_name, color_name_ar, color_hex, stock)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(product_id, size, color_name, color_name_ar || '', color_hex || '#000000', stock || 0);
  ...
  if (Array.isArray(d.items) && d.items.length) {
    ...
    const insertOrder = db.transaction(() => {
      d.items.forEach(item => {
        stmt.run(id, item.productId || '', item.name || '', item.color || '', item.size || '', item.qty || 1, item.price || 0);
        decrStmt.run(item.qty || 1, item.productId || '', item.size || '', item.color || '');
      });
    });
    insertOrder();
  }
...
app.get('/api/stats', auth, (req, res) => {
  ...
  const lowStock       = db.prepare('SELECT COUNT(*) as c FROM product_variants WHERE stock > 0 AND stock <= 5').get().c;
  const outOfStock     = db.prepare('SELECT COUNT(*) as c FROM product_variants WHERE stock = 0').get().c;
  const totalRevenue   = db.prepare("SELECT SUM(total) as t FROM orders WHERE status != 'cancelled'").get().t || 0;
  ...
  res.json({ totalProducts, totalOrders, pendingOrders, lowStock, outOfStock, totalRevenue, recentOrders, lowStockItems, byStatus });
});
...
app.get('/api/public/inventory', (req, res) => {
  const inventory = db.prepare('SELECT product_id, size, color_name as color, stock FROM product_variants').all();
